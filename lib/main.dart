...
class _RotaryState extends State<Rotary> {
  ...
  String _selection = "", display = "", results = "", _operator = "";
  // _history = ""
  double radius = 130;
  double _movement = 0;
  ...
  void check(
    ...
    if (counter == results.length) {
      equal = false;
      print("results unchanged, selection does not equal 24");
    } else {
      equal = true;
    ...
    if (selection.length == 4) {
      for (int i = 1; i < 4; i++) {
        selection.runes.forEach((int rune) {
          ...
          // print("${selection.substring(i)}"); // Used for debugging rune separation
  ...
  void _displaySelection() {
    ...
    if (results.isNotEmpty && equal == true) {
      display = "";
      ...
      if (display[3] != '=') {
        // Reduce garbage data potentially generated by the check() algorithm
